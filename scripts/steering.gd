...
func repel(position, target, repel_radius, current_motion, max_speed):
	var desired_motion = position - target
	var distance = desired_motion.length()

	desired_motion = desired_motion.normalized() * max_speed
	# Already too far, no need to repel.
...
func next_wander_angle(wander_angle, angle_change):
	...
	return fposmod(wander_angle, PI * 2)
